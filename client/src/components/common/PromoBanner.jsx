...
const PromoBanner = ({ products = [] }) => {
  ...
  return (
    <section className={`relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-br ${activePromo.gradient} px-6 py-14 text-white shadow-md sm:px-12 transition-all duration-700`}>
      ...
      {activeProduct?.image && (
        <div className="absolute inset-y-0 right-6 w-2/5 hidden md:flex items-center justify-center pointer-events-none p-4">
          <img
            src={activeProduct.image}
            alt={activeProduct.name}
            className="max-h-full max-w-full object-contain opacity-95 transition-all duration-500 hover:scale-105"
